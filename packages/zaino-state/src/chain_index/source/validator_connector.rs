...
impl ValidatorConnector {
    ...
    /// config, returning the connector, the validator's `getinfo` response (used by
    /// the backend to build its `ServiceMetadata`), and the runtime network whose
    /// activation schedule was adopted from the validator (zaino#1076).
    ///
    ...
    ) -> Result<(Self, GetInfoResponse, zebra_chain::parameters::Network), BlockchainSourceError>
    {
        ...
        let network = adopt_network(common, &fetcher).await?;
    ...
    ) -> Result<(Self, GetInfoResponse, zebra_chain::parameters::Network), BlockchainSourceError>
    {
        ...
        let network = adopt_network(common, &rpc_client).await?;
