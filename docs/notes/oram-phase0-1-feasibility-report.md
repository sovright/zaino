...
- the separate private outer security-state commit foundation does not
  ...
  conditions. A module-private construction and verification foundation
  defines a versioned, domain-separated composite security-component digest
  that currently commits the replay-journal digest. Initial provisioning is
  explicit; its restart-verification seam requires an exact
  outer-snapshot/current-replay match, and live successor construction requires
  an explicitly retained pre-advance replay digest and reads the real current
  digest from a ready post-advance journal. It does not infer direction or
  repair either side, and a journal latched indeterminate cannot supply
  component state. Supplying the same authoritative instance and enforcing the
  allowed commit count remain coordinator obligations. No non-test caller can
  construct and coordinate both private stores in this slice. The journal still
  has only a deterministic test protector, a bound not yet derived from the
  compiled profile, no process lock for its assumed single writer, and no
  runtime caller. Witness advancement is not coordinated and journal/snapshot
  persistence is not one atomic transaction. No concrete witness, nonce/time
...
| Persistence/recovery/RTO | Partial public-rebuild, private outer-commit, replay-journal, and component-binding foundations; production gate blocked | Fixed authenticated public manifests, exact digest-bound external freshness transitions, deterministic crash-boundary tests, and fresh-worker genesis replay establish a fail-closed public publication/rebuild contract. A separate private fixed-width snapshot commits stable security identity plus opaque serving/component digests locally before an injected exact witness. The local replay component journal adds fixed-size context-sealed transactions, exact committed-prefix reconstruction, uniform replacement of the unopened next candidate, and real phase-prefix tests. A module-private construction and verification foundation defines a versioned, domain-separated composite digest that currently commits the replay-journal digest: provisioning is explicit, its restart-verification seam requires an exact outer/current-replay match, and successor construction requires a retained pre-advance digest plus the real current digest read from a ready post-advance journal. The coordinator must supply the same authoritative instance and enforce the allowed commit count; the binding performs no direction inference or repair. No non-test caller can construct and coordinate both private stores in this slice. The journal remains runtime-unwired, single-writer without a lock, protected only by a test fixture, and not profile-derived; snapshot/journal/witness advancement is not one coordinated atomic transaction. The source-bound runner still measures only allocation through validated readiness. The candidate ROSTL adapter remains volatile; source cache is uncontrolled, and no production witness/key/nonce owner, nonce/time journal, qualified integrated replay, composite ORAM-state commit, full-corpus result, controlled-cache result, or full-service RTO exists | Bind replay capacity/cadence in profile v4, coordinate replay/snapshot/witness advancement under production freshness/key ownership, add nonce/time roots, then either implement authenticated atomic ORAM persistence or run a controlled target-hardware rebuild and publish an accepted full-service RTO |
...
replay journal, and a module-private construction and verification foundation
between their value contracts. The versioned, domain-separated composite
security-component digest currently commits the replay-journal component
digest. Explicit provisioning constructs the initial outer snapshot, the
restart-verification seam accepts only an exact match with the current replay
state, and successor construction requires a retained pre-advance replay digest
and reads the real current digest from a ready post-advance journal. A journal
latched indeterminate cannot supply component state. Supplying the same
authoritative instance and enforcing the allowed commit count remain
coordinator obligations. No non-test caller can construct and coordinate both
private stores in this slice. The binding never infers which component is ahead
or repairs either component. The replay commit, outer replacement, and witness
advancement are not coordinated as one atomic transaction.
