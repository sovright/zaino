...
- the separate private outer security-state commit foundation does not
  ...
  conditions. It has only a deterministic test protector, a bound not yet
  derived from the compiled profile, no outer component-digest binding, no
  process lock for its assumed single writer, and no runtime caller. No
  concrete witness, nonce/time component journal, production key/nonce owner,
  or integrated replay provider exists;
...
| Persistence/recovery/RTO | Partial public-rebuild, private outer-commit, and local replay-journal foundations; production gate blocked | Fixed authenticated public manifests, exact digest-bound external freshness transitions, deterministic crash-boundary tests, and fresh-worker genesis replay establish a fail-closed public publication/rebuild contract. A separate private fixed-width snapshot commits stable security identity plus opaque serving/component digests locally before an injected exact witness. The local replay component journal adds fixed-size context-sealed transactions, exact committed-prefix reconstruction, uniform replacement of the unopened next candidate, and real phase-prefix tests. It remains unwired, single-writer without a lock, protected only by a test fixture, unbound from the outer digest, and not profile-derived. The source-bound runner still measures only allocation through validated readiness. The candidate ROSTL adapter remains volatile; source cache is uncontrolled, and no production witness/key/nonce owner, nonce/time journal, qualified integrated replay, composite ORAM-state commit, full-corpus result, controlled-cache result, or full-service RTO exists | Bind and qualify replay plus nonce/time roots under production freshness/key ownership, then either implement authenticated atomic ORAM persistence or run a controlled target-hardware rebuild and publish an accepted full-service RTO |
