...
logical sizing now exist. A separate source-bound artifact is a typed NO-GO for
the exact current 1x-capacity, four-probe profile under eight fixed schedules
and a zero-bps sampled failure budget. Gate 1 remains open on a remediated
design or accepted alternative-profile evidence, growth, an accepted failure
bound, backend calibration, and target-TDX RSS/headroom; compiled obliviousness
also remains failed at the recorded exact evidence source. The pinned
dependency manifests declare
`MIT OR Apache-2.0`, while authoritative repository license and notice files
remain unconfirmed; this is tracked as distribution-readiness due diligence,
...
[mainnet insertion-bound log](./docs/notes/oram-gate1-mainnet-insertion-bound-log-2026-07-27.md),
[checked-in ORAM evidence convention](./docs/evidence/oram/README.md),
